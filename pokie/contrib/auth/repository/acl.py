...
class AclRoleRepository(Repository):
    ...
    def can_remove(self, id_role: int) -> bool:
        ...
        sql, values = Select(self._dialect) \
            .from_(AclUserRole, cols={Literal('COUNT(*)'): 'total'}, schema=self._schema) \
    ...
    def add_role_resource(self, id_role: int, id_resource: int):
        ...
        sql, values = Insert(self._dialect) \
    ...
    def add_user_role(self, id_user, id_role):
        ...
        sql, values = Insert(self._dialect).into(AclUserRole(id_role=id_role, id_user=id_user)).assemble()
        self.exec(sql, values)
...
class AclResourceRepository(Repository):
    ...
    def find_user_resources(self, id_user: int) -> List[AclResource]:
        ...
        if not sql:
            sql, _ = self.select() \
                .join(AclRoleResource, AclRoleResource.id_resource, AclResource, AclResource.id) \
                .join(AclUserRole, AclUserRole.id_role, AclRoleResource, AclRoleResource.id_role) \
                .where(AclUserRole.id_user, '=', id_user) \
                .assemble()
            self._cache_set(key, sql)
