    ...
    def run(self, args) -> bool:
        ...
        for _, module in di.get(DI_APP).modules.items():
            for cmd, cmd_path in module.cmd.items():
                ...
                if not cls:
                    raise RuntimeError(
                        "Error: class '{}' not found while listing available CLI commands".format(cmd_path))
...
class ListCmd(BaseCommand):
    ...
    def run(self, args) -> bool:
        ...
        for cmd, cmd_path in self.get_cmd_map().items():
            ...
            # show details
            self.tty.write("{} \t {}".format(color.green(cmd), color.white(cls.description)).expandtabs(32))
...
class RunServerCmd(CliCommand):
    ...
    def arguments(self, parser: ArgumentParser):
        parser.add_argument('-h', '--host', help="The interface to bind to (default: 127.0.0.1)", required=False,
                            default='127.0.0.1')
        parser.add_argument('-p', '--port', help="The port to bind to (default: 5000)", required=False, default='5000')
        parser.add_argument('-d', '--debug', help="Enable debug mode (default: false)", required=False,
                            action='store_true',
                            default=False)
        parser.add_argument('-r', '--reload', help="Enable automatic reload (default: false)", required=False,
                            action='store_true',
                            default=False)
