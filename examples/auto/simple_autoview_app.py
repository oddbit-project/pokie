...
# because Auto.view() will require database access to the table customers, we need to postpone route registration
# to the actual web initialization routines, as it is done with modules
def router(p: FlaskApplication):
    # Auto.view() will generate a view for the customers table and register the following endpoints:
